...
app.put('/zadatak/:id', (req, res) => {
    ...
    if (!naziv || !opis) {
        return res.status(400).json({ status: "Naziv i opis su obavezni!" });
    }
    ...
    fs.readFile(filePath, 'utf8', (err, data) => {
        ...
        const lines = data.split('\n');
        const taskIndex = lines.findIndex(line => line.split(',')[0].trim() === taskId);

        if (taskIndex === -1) {
            return res.status(404).json({ status: "Id ne postoji!" });
        }

        lines[taskIndex] = `${taskId},${naziv},${opis}`;

        fs.writeFile(filePath, lines.join('\n'), err => {
            if (err) {
                return res.status(500).json({ status: "Greška pri ažuriranju zadatka!" });
            }
